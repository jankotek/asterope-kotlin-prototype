...
public class PixToolTest1 extends TestCase {
    ...
    public void testVertexes(){
        ...
        Vector3D[] vertexes = tools.makePix2Vect(ipix).toVertex();
        long ipixR = tools.ang2pix( theta, phi);
        Vector3D[] vertexesr = tools.makePix2Vect(ipixR).toVertex();
        for (int i=0; i< 4; i++) {
            Vector3D vect = vertexes[i];
            double[] angs =PixToolsUtils.Vect2Ang(vect);
            Vector3D vectR = vertexesr[i];
