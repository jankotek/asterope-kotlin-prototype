 ...
 */
public class SubsetMosaicker implements Processor {
 
    private java.util.HashSet<String> usedImageNames = new java.util.HashSet<String>();
	...
	for (int i=0; i<subsets.length; i += 1) {
	    ...
	    processSubset(input, subsets[i], osrcs[i], samp, dSampler);
	}
	
    }
    public void processSubset(Image[] input, Image output, int[] osource,
			      Sampler samp, DepthSampler dSampler) {
