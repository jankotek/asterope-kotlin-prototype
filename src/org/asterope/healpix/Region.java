...
//	private boolean normalized = false;
	...
	public Region(double xMin, double xMax, double yMin, double yMax) {
		...
		this.phiMin = PixToolsUtils.MODULO(this.phiMin, TWOPI) - epsilon; // phi min, excluding 2pi period
		this.phiMax = PixToolsUtils.MODULO(this.phiMax, TWOPI) + epsilon;
		...
		this.tetMax = FastMath.PI/2. - Math.toRadians(decMin);
		this.tetMin = FastMath.PI/2. - Math.toRadians(decMax);
...
//		double racomp = ra;
		...
		phiComp = PixToolsUtils.MODULO(phiComp, TWOPI) - epsilon;
...
//		racomp = Math.toDegrees(phiComp);
	...
	public boolean inRegPol(double phi, double theta) {
		...
		double  phicomp = phi;
		if (phicomp < 0.) phicomp += TWOPI;
		phicomp = PixToolsUtils.MODULO(this.phiMin, TWOPI) - epsilon;
