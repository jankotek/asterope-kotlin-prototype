...
public class PixToolsUtils {
	...
	public static double SurfaceTriangle(Vector3D v1, Vector3D v2, Vector3D v3)
		...
		double x0 = FastMath.tan(side1 + side2 + side3);
		double x1 = FastMath.tan(side2 + side3 - side1);
		double x2 = FastMath.tan(side1 + side3 - side2);
		double x3 = FastMath.tan(side1 + side2 - side3);
		return 4.0 * FastMath.atan(FastMath.sqrt(x0 * x1 * x2 * x3));
	}
    ...
    public static double[] RaDecToPolar(double[] radec) {
    	double[] res = {0.0,0.0};
			...
			double ra =  radec[0];
			double dec =  radec[1];
			double theta = FastMath.PI/2. - FastMath.toRadians(dec);
			double phi = FastMath.toRadians(ra);
			res[0] = theta;
			res[1] = phi;
    ...
    public static double[] PolarToRaDec(double[] polar) {
    	double[] radec = {0.0,0.0};
			...
			double dec = FastMath.toDegrees(FastMath.PI/2. - theta);
			double ra = FastMath.toDegrees(phi);
			radec[0] = ra;
			radec[1] = dec;
	...
	public static double[] Vect2Ang(Vector3D v) {
		...
		double theta = FastMath.acos(z);
		double phi = 0.;
		if ((v.getX() != 0.) || (v.getY() != 0)) {
			phi = FastMath.atan2(v.getY(), v.getX()); // phi in [-pi,pi]
		}
		if (phi < 0)
			phi += 2.0 * FastMath.PI; // phi in [0, 2pi]
//		phi += FastMath.PI;
		out_tup[0] = theta;
		out_tup[1] = phi;
		return out_tup;
