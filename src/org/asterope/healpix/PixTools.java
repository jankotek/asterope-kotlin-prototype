...
//	private static double FOURPI = 4. * PI;
    ...
    public LongRangeSet query_strip(double theta1, double theta2) throws Exception {
        ...
        for (is = 0; is < nstrip; is++) {
            irmin = RingNum(FastMath.cos(colrange[2 * is]));
            irmax = RingNum(FastMath.cos(colrange[2 * is + 1]));
            /* loop on ring number */
...
//		npix = Nside2Npix(nside);
    ...
    public LongRangeSet query_triangle(
        ...
        if (FastMath.abs(determ) < 1.0e-20) {
        ...
        for (int i = 0; i < 3; i++) {
            sto[i] = FastMath.sqrt((1.0 - vo[i].getZ()) * (1.0 + vo[i].getZ()));
        }
        /*
        ...
        zmax = FastMath.max(FastMath.max(vv[0].getZ(), vv[1].getZ()), FastMath.max(vv[2].getZ(), zmax));
        zmin = FastMath.min(FastMath.min(vv[0].getZ(), vv[1].getZ()), FastMath.min(vv[2].getZ(), zmin));
        /*
        ...
        if (do_inclusive) {
            ...
            sin_off = FastMath.sin(offset);
            zmax = FastMath.min(1.0, FastMath.cos(FastMath.acos(zmax) - offset));
            zmin = FastMath.max(-1.0, FastMath.cos(FastMath.acos(zmin) + offset));
        }
        ...
        for (int j = 0; j < 3; j++) {
            if (sto[j] > 1.0e-10) {
                ...
                phi0i[j] = FastMath.atan2(vo[j].getY(), vo[j].getX()); // Should make it 0-2pi
                // ?
                /* Bring the phi0i to the [0,2pi] domain if need */

                if (phi0i[j] < 0.) {
                    phi0i[j] = PixToolsUtils.MODULO(
                            (FastMath.atan2(vo[j].getY(), vo[j].getX()) + TWOPI), TWOPI); //  [0-2pi]
                }
...
//		double temp = 0.;
        ...
        for (long iz = irmin; iz <= irmax; iz++) {
            ...
            final double st = FastMath.sqrt((1.0 - z) * (1.0 + z));
            ...
            for (int k = 0; k < 3; k++) {
                ...
                if (dc[k] * sdet <= -1.0) { // the whole iso-latitude ring is on
                    ...
                    double phi_neg = phi0i[k] - (FastMath.acos(dc[k]) * sdet);
                    double phi_pos = phi0i[k] + (FastMath.acos(dc[k]) * sdet);
                    //
                    if (phi_pos < 0.)
                        phi_pos += TWOPI;
                    if (phi_neg < 0.)
            ...
            if (n12 != 0) {
                ...
                if (!found) {
                    for (long idom = 0; idom < ndom; idom++) {
                        ...
                        double dphiring = FastMath.abs(b_i - a_i) / 2.0;
    ...
    public LongRangeSet query_disc(Vector3D vector, double radius,
        ...
        double cosang = FastMath.cos(radius_eff);
        /* disc center */
...
//		System.out.println("x0="+x0+" y0="+y0+" z0="+z0);
        ...
        double dphi = 0.0;
        if (x0 != 0. || y0 != 0.)
            phi0 = PixToolsUtils.MODULO(FastMath.atan2(y0, x0) + TWOPI, TWOPI);  // in [0, 2pi]
        double cosphi0 = FastMath.cos(phi0);
//			System.out.println("phi0="+phi0+" cosphi0="+cosphi0);
        double a = x0 * x0 + y0 * y0;
        /* coordinate z of highest and lowest points in the disc */
        double rlat0 = FastMath.asin(z0); // latitude in RAD of the center
        ...
        if (rlat1 >= HALFPI) {
            ...
            zmax = FastMath.sin(rlat1);
        }

        long irmin = FastMath.max(1, RingNum(zmax) - 1); // start from a higher point to be safe
        if (rlat2 <= -HALFPI) {
            zmin = -1.0;
        } else {
            zmin = FastMath.sin(rlat2);
        }

        long irmax = FastMath.min(4 * nside - 1, RingNum(zmin) + 1); // go down to a lower point
...
//		ilist = -1;
        ...
        for (long iz = irmin; iz <= irmax; iz++) {
            ...
            double c = 1.0 - z * z;
            double cosdphi = b / FastMath.sqrt(a * c);
            long done = 0;

            if (FastMath.abs(x0) <= 1.0e-12 && FastMath.abs(y0) <= 1.0e-12) {
                cosdphi = -1.0;
                dphi = PI;
                done = 1;
            }
            if (done == 0) {
                if (FastMath.abs(cosdphi) <= 1.0) {
                    dphi = FastMath.acos(cosdphi); // in [0,pi]
                } else {
                    if (cosphi0 >= cosdphi) {
...
//
        if (res.size() == 0 && pixres > FastMath.toDegrees(radius)/3600.) {
...
//this is HOTFIX for rounding errors in InRing method
    ...
    public double[] pix2ang(long ipix)  {
        ...
        if (ipix1 <= ncap) { // North polar cap
            ...
            long iring = (long) (FastMath.sqrt(hip - FastMath.sqrt(fihip))) + 1; // counted from north
            // pole
            long iphi = ipix1 - 2 * iring * (iring - 1);
            theta = FastMath.acos(1.0 - iring * iring / (3.0 * nsidesq));
            ...
            long iphi = ip% nl4 + 1;
            double fodd = 0.5 * (1. + PixToolsUtils.MODULO(iring + nside, 2)); // 1 if iring+nside
            // is odd, 1/2 otherwise
            theta = FastMath.acos((nl2 - iring) / (1.5 * nside));
            ...
            long iring = (long) (FastMath.sqrt(hip - FastMath.sqrt(fihip))) + 1; // counted from South
            // pole
...
//		boolean do_vertex = true;
        ...
        /* pixel center */
        double sth = FastMath.sqrt((1.0 - z) * (1.0 + z));
        //pixVect.getX() = sth * FastMath.cos(phi);
        //pixVect.getY() = sth * FastMath.sin(phi);
        //pixVect.getZ() = z;
        Vector3D pixVect = new Vector3D(sth * FastMath.cos(phi), sth * FastMath.sin(phi), z);
        /* west vertex */
        double phi_wv = phi - hdelta_phi;

        Vector3D west = new Vector3D(sth * FastMath.cos(phi_wv), sth * FastMath.sin(phi_wv),z);

        /* east vertex */
        double phi_ev = phi + hdelta_phi;
        Vector3D east = new Vector3D(sth * FastMath.cos(phi_ev),  sth * FastMath.sin(phi_ev), z);
        /* north vertex */
        double sth_nv = FastMath.sqrt((1.0 - z_nv) * (1.0 + z_nv));
        Vector3D north = new Vector3D(sth_nv * FastMath.cos(phi_nv), sth_nv * FastMath.sin(phi_nv), z_nv);
        /* south vertex */
        double sth_sv = FastMath.sqrt((1.0 - z_sv) * (1.0 + z_sv));
        Vector3D south = new Vector3D(sth_sv * FastMath.cos(phi_sv), sth_sv * FastMath.sin(phi_sv),  z_sv);
    ...
    public long vect2pix(Vector3D vector)  {
        ...
        if (vector.getX() != 0. || vector.getY() != 0.)
            phi = FastMath.atan2(vector.getY(), vector.getX()); // phi in [-pi,pi]
    ...
    private long zPhi2Pix(double z, double phi) {
        ...
        if (za < twothird) { // equatorial region
            ...
            long kshift = 0;
            if ( PixToolsUtils.MODULO(ir, 2) == 0)
            ...
            double tmp = FastMath.sqrt(3.0 * (1.0 - za));
...
//		long inext;
        ...
        // cap
        double phi_low = PixToolsUtils.MODULO(phi0 - dphi, TWOPI) - epsilon; // phi min,
        // excluding
        // 2pi period
        double phi_hi = PixToolsUtils.MODULO(phi0 + dphi, TWOPI) + epsilon;
        ...
        if ((iz >= nside) && (iz <= 3 * nside)) { // equatorial region
            ...
            kshift = (long) PixToolsUtils.MODULO(ir, 2.);
        ...
        if (FastMath.abs(dphi - PI) < epsilon) {
            //take entire range
        ...
        if (conservative) {
            ip_low = FastMath.round((nr * phi_low) / TWOPI - shift);
            ip_hi = FastMath.round((nr * phi_hi) / TWOPI - shift);
...
//			System.out.println("ip_low="+ip_low+" ip_hi="+ip_hi);
            ...
            ip_low = (long) FastMath.ceil((nr * phi_low) / TWOPI - shift);
            ...
            ip_low = FastMath.min(ip_low, nr - 1);
            ip_hi = FastMath.max(ip_hi, 0);
        }
    ...
    public long RingNum(double z) {
        ...
        long iring = FastMath.round(nside * (2.0 - 1.5 * z));
        /* north cap */
        if (z > twothird) {
            iring = FastMath.round(nside * FastMath.sqrt(3.0 * (1.0 - z)));
            if (iring == 0)
                iring = 1;
        }
        /* south cap */
    ...
    public static Vector3D Ang2Vec(double theta, double phi)  {
        ...
        double stheta = FastMath.sin(theta);
        double x = stheta * FastMath.cos(phi);
        double y = stheta * FastMath.sin(phi);
        double z = FastMath.cos(theta);
    ...
    public static long Npix2Nside(long npix) {
        ...
        long nside = (long) FastMath.rint(FastMath.sqrt(npix / 12));
        if (npix < 12) {
        ...
        if (FastMath.abs(fnpix - npix) > 1.0e-2) {
            throw new IllegalArgumentException("npix is not 12*nside*nside");
        }
        double flog = FastMath.log((double) nside) / FastMath.log(2.0);
        double ilog = FastMath.rint(flog);
        if (FastMath.abs(flog - ilog) > 1.0e-6) {
    ...
    public static double PixRes(long nside) {
        double degrad = FastMath.toDegrees(1.0);
        ...
        res = FastMath.sqrt(res);           // angular size of the pixel arcsec
        return res;
    }
    /**
    ...
    public static long GetNSide(double pixsize) {
        ...
        double degrad = FastMath.toDegrees(1.);
        double skyArea = 4.*PI*degrad*degrad*3600.*3600.;
        long npixels = (long) (skyArea/pixelArea);
        long nsidesq = npixels/12;
        long nside_req = (long) FastMath.sqrt(nsidesq);
        long mindiff = ns_max;
        int indmin = 0;
        for (int i=0; i<nsidelist.length; i++) {
            if (FastMath.abs(nside_req - nsidelist[i]) <= mindiff) {
                mindiff = FastMath.abs(nside_req - nsidelist[i]);
                res = nsidelist[i];
                indmin = i;
            }
